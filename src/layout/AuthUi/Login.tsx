...
import { useCookiesFunc } from '@/utils/cookiesFunc';


function Login() {
    const navigate = useNavigate();
    const [handleLoginApi, { isLoading, isSuccess, error, data }] = useLoginApiMutation();
    const { saveCookie } = useCookiesFunc()
    ...
    useEffect(() => {
        if (isSuccess) {
            resetForm();
            saveCookie({ key: "refreshToken", value: data?.data?.refreshToken, isEncrypted: true });
            saveCookie({ key: "userName", value: data?.data?.userName });
            saveCookie({ key: 'email', value: data?.data?.email });
            navigate('/chat')
        }
    }, [isSuccess])
